...
void getDateTime(char* buffer, size_t bufferSize)
{
  struct timeval tv;
  ...
  char temp[25];
  strftime(temp, sizeof(temp), "%Y-%m-%dT%H:%M:%S", &timeinfo);
  snprintf(buffer, bufferSize, "%s.%03dZ", temp, millisec);
}
...
{
  getDateTime(dateTimeStr, sizeof(dateTimeStr));
  ...
  snprintf(telemetry_payload, sizeof(telemetry_payload),
    "{ \"datetime\": \"%s\", \"humidity\": %.2f, \"temperature\": %.2f }",
    dateTimeStr, humidity, temperature);
}
...
static void sendTelemetry()
{
  ...
  az_iot_message_properties_init(&properties, az_span_create((uint8_t*)property_buffer, sizeof(property_buffer)), 0);
  
  // Set content type to application/json
  if (az_result_failed(az_iot_message_properties_append(
          &properties, 
          AZ_SPAN_FROM_STR("$.ct"), 
          AZ_SPAN_FROM_STR("application/json"))))
  {
    Logger.Error("Failed to set content type");
  }
  ...
  {
    Logger.Error("Failed to set content encoding");
  }
